...
namespace smartGPS.Business.Custom
{
    public class Haversine
    {
        ...
        public double Distance(double startLat, double startLong, double endLat, double endLong, DistanceType type)
        {
            SmartLocation pos1 = new SmartLocation(startLat, startLong);
            SmartLocation pos2 = new SmartLocation(endLat, endLong);
            double preDlat = pos2.Latitude - pos1.Latitude;
            double preDlon = pos2.Longitude - pos1.Longitude;
            double R = (type == DistanceType.Miles) ? 3960 : 6371;
            double dLat = this.toRadian(preDlat);
